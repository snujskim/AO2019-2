...
# although I could pick more points, they made error too big
...
OBJIMAGE = Path(top / "neptune_0_bdf.fits")
objhdu = fits.open(OBJIMAGE)
...
_.writeto(top/(OBJIMAGE.stem+".skysub.fits"),overwrite=True) 
...
axs[0].imshow(objimage, vmin=0, vmax=200, origin='lower')
...
sensfunc = pydis.DefFluxCal(ap_wavelen_h, ap_summed_h, stdstar='mhr9087.dat',
                  mode='spline', display=True)


#%%

ffinal,efinal = pydis.ApplyFluxCal(ap_wavelen, ap_summed, 0,
                                    ap_wavelen_h, sensfunc)
#
#fig,ax=plt.subplots(1,1,figsize=(10,8))
#ax.plot(ap_wavelen, ffinal)
##ax.set(ylim=(0,0.025*1e-11))
#plt.show()

plt.figure(figsize=(10,8))
plt.plot(ap_wavelen, ffinal, color='blue')
plt.errorbar(ap_wavelen, ffinal, yerr=efinal)
plt.xlabel('Wavelength')
plt.ylabel('Flux (erg/s/cm2/A)')
plt.title('Neptune Calibrated')
#plot within percentile limits
plt.xlim(4000,)
plt.ylim( (np.percentile(ffinal,2),
           np.percentile(ffinal,99)) )
plt.show()
...
#lowercut_ID = N_SPATIAL//2 - NSUM_ID//2 
#uppercut_ID = N_SPATIAL//2 + NSUM_ID//2
#identify_1 = np.median(ffinal, axis=0)
#x_identify = np.arange(0, len(ffinal))
#
#lampimage = set_dispaxis(ffinal, DISPAXIS)
#
#lowercut_ID = N_SPATIAL//2 - NSUM_ID//2 
#uppercut_ID = N_SPATIAL//2 + NSUM_ID//2
#identify_1 = np.median(lampimage[lowercut_ID:uppercut_ID, :], axis=0)
...
# For plot and visualization
MINAMP_PK = 0.001
fffinal = np.array(ffinal)
max_intens = np.max(fffinal)

peak_pix = peak_local_max(fffinal, indices=True, num_peaks=NMAX_PK,
                          min_distance=MINSEP_PK,
                          threshold_abs=max_intens * MINAMP_PK)
# ``peak_pix`` corresponds to the x value, since x = pixels starting from 0.
 #disable_mplkeymaps()
 
app_wavelen = ap_wavelen[peak_pix] 

fig = plt.figure(figsize=(10,10))
ax = fig.add_subplot(111)
title_str = r'Peak ($A \geq {:.2f} A_\mathrm{{max}}$, $\Delta x \geq {:.0f}$)'
# Plot original spectrum + found peak locations
ax.plot(ap_wavelen, fffinal, lw=1)

for i in app_wavelen:
    ax.plot((i, i), 
            (fffinal[0]+0.001*max_intens, fffinal[0]+0.005*max_intens),
            color='r', ls='-', lw=1)
    ax.annotate(i, (i, 0.9),
                xycoords = ('data', 'axes fraction'),
                fontsize='small', rotation=70)
...
ax.set_ylabel('Pixel value sum')
ax.set_xlim(4000,)
ax.set_ylim( (np.percentile(fffinal,2),np.percentile(fffinal,99)))
ax.set_title(title_str.format(MINAMP_PK, MINSEP_PK))
plt.show()
...
response = JPLSpec.query_lines(min_frequency=10000 * u.GHz,
                                   max_frequency=100000 * u.GHz,
                                   min_strength=-500,
                                   molecule="26002 C2H2",
                                   #max_lines = 7,
